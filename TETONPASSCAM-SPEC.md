...
- One scheduled worker, **seasonal cadence:** every 15 minutes in summer, every 5–10 minutes in winter (Nov–Apr) — WYDOT's own refresh is 300s, so never faster than 5 min. Each run: fetch WYDOT primary + weather → upsert `status`; fetch Routes API per route-direction → insert into `travel_times`; poll Idaho 511 events (respect its 10-calls/60s throttle — one call per cycle is plenty). **Amendment (LH T1, 2026-08-10):** the fallback page is fetched every cycle, not only "on disagreement/failure" as originally written here — a primary-only fetch can never catch a primary/fallback disagreement in the first place. When primary and fallback disagree on open-vs-closed, consult the Statewide crosscheck; if that still can't resolve it → UNKNOWN.
